...
if (!empty($_GET['solution_category'])) {
    $args['tax_query'] = [['taxonomy' => 'solution_category', 'field' => 'slug', 'terms' => sanitize_text_field($_GET['solution_category'])]];
}
...
<section class="archive-hero">
    <div class="container">
        ...
        <nav class="breadcrumb">
            <a href="<?php echo home_url(); ?>">Home</a> <span>/</span> Solutions
        </nav>
    </div>
</section>
...
<section class="archive-main">
    <div class="container">
        ...
        <div class="filter-tabs">
            <a href="?" class="<?php echo empty($_GET['solution_category']) ? 'active' : ''; ?>">All</a>
            <?php
            // Get IDs of all solutions to filter relevant categories
            $solution_ids = get_posts(['post_type' => 'solution', 'posts_per_page' => -1, 'fields' => 'ids']);
            $terms = get_terms(['taxonomy' => 'solution_category', 'hide_empty' => true, 'object_ids' => $solution_ids]);

            if (!is_wp_error($terms) && !empty($terms)) {
                foreach ($terms as $term) {
                    $active = (isset($_GET['solution_category']) && $_GET['solution_category'] == $term->slug) ? 'active' : '';
                    echo '<a href="?solution_category='.esc_attr($term->slug).'" class="'.$active.'">'.esc_html($term->name).'</a>';
                }
            }
            ?>
        </div>
        ...
        <div class="archive-grid">
            <?php if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post(); ?>
                ...
                <?php 
    $image = get_field('banner_image'); // Replace with your field name
    if( !empty($image) ): ?>
    ...
    <?php endif; ?>
                    ...
                    <div class="card-content">
                        ...
                        <a href="<?php the_permalink(); ?>" class="card-link">Explore Solution →</a>
                    </div>
                </article>
